...
public class AddItemDialog extends JDialog {
    ...
    public AddItemDialog(JFrame parent, ArrayList<Item> items) {
    ...
    private void saveItem() {
        try {
            ...
            Item item;
            if (typeCombo.getSelectedIndex() == 1) { // Discount item
                double discount = Double.parseDouble(discountField.getText().trim());
                if (discount < 0 || discount > 100) {
                    JOptionPane.showMessageDialog(this, "Discount must be between 0 and 100!");
                    return;
                }
                item = new DiscountItem(id, name, price, discount, stock);
            } else { // Standard item
                item = new StandardItem(id, name, price, stock);
            }
