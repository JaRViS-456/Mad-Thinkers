    ...
    private void handleLogin(boolean isAdmin) {
        ...
        if (user != null && user.authenticate(password)) {
            if (isAdmin && user instanceof Admin) {
                new AdminDashboard((Admin) user, users, items).setVisible(true);
                dispose();
            } else if (!isAdmin && user instanceof Employee) {
                new EmployeeDashboard((Employee) user, items).setVisible(true);
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, 
                    isAdmin ? "User is not an Admin!" : "User is not an Employee!",
                    "Login Error", JOptionPane.ERROR_MESSAGE);
            }
        } else {
